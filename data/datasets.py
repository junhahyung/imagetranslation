...
class CelebABase(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = self.default_loader(os.path.join(self.subdir, self.filenames[index]))
...
if __name__ == '__main__':
    ...
    celeba = CelebAAligned4MAFLVal(config)
    print(len(celeba))
    mafl_train = MAFLAligned(config)
    print(len(mafl_train))
    mafl_test = MAFLAligned(config, train=False)
    print(len(mafl_test))
