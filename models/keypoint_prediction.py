...
class IntermediateKeypointPredictor(nn.Module):
    def __init__(self, config, input_dim, numerical_check=False):
        super().__init__()
        args = config['keypoint_regressor']['args']
        self.nA = args['num_annotated_points']
        self.nI = args['num_intermediate_points']
        self.softargmax_mul = args['softargmax_mul']
        self.descriptor_dimension = input_dim

        weights = torch.randn(self.descriptor_dimension, self.nA, self.nI)
        linearlist = nn.ModuleList([
            nn.Linear(self.nI * 2, 2, bias=False)
        ...
        self.inner_conv = nn.Conv2d(
            in_channels=self.descriptor_dimension,
            out_channels=latent_dim,
            kernel_size=1,
            bias=False,
        )
        weights = weights.view(self.descriptor_dimension, -1).t()
        weights = weights.view(latent_dim, self.descriptor_dimension, 1, 1)
