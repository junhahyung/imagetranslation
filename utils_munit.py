...
def get_all_data_loaders(conf):
    ...
    if 'data_list_train_a' not in conf:
        ...
        test_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'testA'), batch_size, False,
                                               new_size_a, new_size_a, new_size_a)
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
    ...
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                          (0.5, 0.5, 0.5))]
    transform_list = [transforms.RandomCrop((height, width))] + transform_list if crop else transform_list
    transform_list = [transforms.Resize(new_size)] + transform_list if train else transform_list
    transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list
    transform = transforms.Compose(transform_list)
...
def get_instance(module, config, category, subcategory, *args, **kwargs):
    return getattr(module, config[category][subcategory])(*args, config, **kwargs)
